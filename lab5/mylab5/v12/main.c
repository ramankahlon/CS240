			...
			else if(a==3)//execute only if "grade" is entered
			{
				...
				//printf("works: \n");//debugging
  				...
  				lt = 0;
                        	while(command[lt]!='\0')
					lt++;
                                if(lt == 0)//if length of command is 0, then skip the system call
                                	continue;

                                //buf[len-1] = '\0';//debugging

				//printf("%d",1);//debugging
				...
				//printf("fork = %d",k);//debugging
	                        ...
	                        if (k==0)//if in child process
                                {
                                        //printf("%d",2);//debugging
					fprintf(stdout,"new process will run: %s\n",command);//print out which new process will run
                                        execlp(command,command,NULL);//run requested command
